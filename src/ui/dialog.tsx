...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn("fixed inset-0 z-[100] bg-black/50 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0", className)}
    {...props}
  />
));
...
>(({ className, children, hideClose, style, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-[50%] top-[50%] z-[100] grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border p-6 shadow-lg rounded-xl",
        className
      )}
      // Theme-aware: community-app sets --bg-color/--text-color per community; the
      // admin (which doesn't) falls back to the original white/zinc light look.
      style={{
        background: "var(--bg-color, #ffffff)",
        color: "var(--text-color, #18181b)",
        borderColor: "color-mix(in srgb, var(--text-color, #18181b) 12%, transparent)",
        ...style,
      }}
      {...props}
    >
